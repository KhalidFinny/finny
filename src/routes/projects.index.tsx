  ...
  return (
    <>
      ...
      >
        ...
        <div role="tablist" aria-label="Project views" className="border-b border-line bg-canvas px-4 py-2.5 md:px-6">
          <div className="motion-enter motion-step-2 flex items-center gap-2">
            {TABS.map((item) => (
              <button
                ...
                onClick={() => {
                  if (item.id !== tab) {
                    animateOutThen(() => setTab(item.id), panelRef.current)
                  }
                }}
                className={`rounded-md px-3 py-1.5 text-sm font-medium uppercase tracking-[0.14em] transition-all duration-200 ${
                  tab === item.id ? 'bg-paper text-brand' : 'text-graphite hover:text-ink'
                }`}
              >
                {item.label}
              </button>
        ...
        <main className={`min-h-0 flex-1 border-b border-line animate-[page-in_300ms_ease-out] motion-reduce:animate-none ${tab === 'programming' ? 'overflow-y-auto md:overflow-hidden' : 'overflow-y-auto'}`}>
          <div key={tab} ref={panelRef} className="h-full animate-[page-in_250ms_ease-out] motion-reduce:animate-none">
            {tab === 'programming' ? (
